...
class RoundRectDrawableWithShadow extends Drawable {
    // used to calculate content padding  Math.cos()用于计算余弦，返回的是 -1.0 到 1.0 之间的数
    private static final double COS_45 = Math.cos(Math.toRadians(45));// Math.toRadians()用于将以度为单位的角度转换为以弧度测量的大致相等的角度,也就是说将度转化为-π/2到π/2之间的弧度值
...
// 如果没有设置阴影颜色，使用默认颜色
    ...
    private void buildShadowCorners() {
        ...
        // 获取外部设置的当前状态下的阴影颜色
        int startColor = mShadowStartColor.getColorForState(getState(), mShadowStartColor.getDefaultColor());
